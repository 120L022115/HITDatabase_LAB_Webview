			...
			themeBgColor() {
				if (this.dark) {
					...
					if (this.backgroundColor) {
						...
						return this.dark ? 'var(--color-texttip)' : 'var(--color-btn-text)'
					}
				}
				return this.backgroundColor || 'var(--color-btn-text)'
			},
			themeColor() {
				if (this.dark) {
					// 默认值
					if (this.color) {
						...
						return this.dark ? 'var(--color-btn-text)' : 'var(--color-texttip)'
					}
				}
				return this.color || 'var(--color-texttip)'
			},
			navbarHeight() {
...
<style lang="scss" scoped>
	...
	.uni-navbar--shadow {
		box-shadow: 0 1px 6px var(--color-texttip);
	}
